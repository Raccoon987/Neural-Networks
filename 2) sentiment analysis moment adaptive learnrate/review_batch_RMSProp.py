...
class NeuralNetwork():
    ...
    def fit(self, X, Y, learning_rate=10e-2, reg=10e-2, epochs=150, show_fig=False):
        N, D = X.shape
        K = len(set(Y))
        T = y_hot_encoding(Y)
        ...
        for i in range(epochs):
            for j in range(n_batches):
                ...
                self.weights[-1] -= learning_rate * gW5 / (np.sqrt(cache_W[-1]) + eps)
                self.biases[-1] -= learning_rate * gb5 / (np.sqrt(cache_b[-1]) + eps)
                ...
                self.weights[-2] -= learning_rate * gW4 / (np.sqrt(cache_W[-2]) + eps)
                self.biases[-2] -= learning_rate * gb4 / (np.sqrt(cache_b[-2]) + eps)
                ...
                self.weights[-3] -= learning_rate * gW3 / (np.sqrt(cache_W[-3]) + eps)
                self.biases[-3] -= learning_rate * gb3 / (np.sqrt(cache_b[-3]) + eps)
                ...
                self.weights[-4] -= learning_rate * gW2 / (np.sqrt(cache_W[-4]) + eps)
                self.biases[-4] -= learning_rate * gb2 / (np.sqrt(cache_b[-4]) + eps)
                ...
                self.weights[-5] -= learning_rate * gW1 / (np.sqrt(cache_W[-5]) + eps)
                self.biases[-5] -= learning_rate * gb1 / (np.sqrt(cache_b[-5]) + eps)
