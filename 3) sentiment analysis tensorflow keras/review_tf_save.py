...
class HiddenLayer():
    ...
    def layer_forward(self, X):
        return tf.matmul(X, self.params[0]) + self.params[1]
...
class NeuralNetwork():
    ...
    def fit(self, X, Y, learning_rate=10e-7, mu=0.99, decay=0.999, reg=10e-3, epochs=400, batch_size=100, split=True, show_fig=False, print_every=20):
        ...
        for id in range(len(self.hidden_layer_sizes)):
            self.hidden_layers.append(HiddenLayer(M1, self.hidden_layer_sizes[id], id, self.nonlin_functions[id]))
    ...
    def forward(self, X):
        ...
        for h, p in zip(self.hidden_layers, self.dropout[1:]):
            Z = self.nonlinear(h.nonlin_func)(h.layer_forward(Z))
            #Z = h.layer_forward(Z)
            if self.training == True:
                Z = tf.nn.dropout(Z, p)
            print(Z.shape)
        return tf.matmul(Z, self.params[0]) + self.params[1]
