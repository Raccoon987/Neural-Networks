...
def get_data(train_file, test_file, balance=True, drop_lst=False):
    ''' balance: to make equal number of positive and negative reviews
        drop_lst: drop some train reviews by its index  '''
...
def crossValidation(model, data, K=5, learning_rate=10e-6, reg=10e-7, epochs=10001):
    ...
    for k in range(K):
        model.fit(scipy.sparse.vstack((X[:k * sz, :], X[(k * sz + sz):, :])), np.append(Y[:k * sz], Y[(k * sz + sz):]),
                  learning_rate=learning_rate, reg=reg, epochs=epochs)
